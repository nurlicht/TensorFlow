...
class ModelData {
  ...
  static create() {
    const params = InputDataParameters.getDefault();
  ...
  static showScatterPlot(values, inputDataParameters, xLabel, yLabel, height) {
    ...
    TfjsApi.tfvis.render.scatterplot(
      ...
      data,
      {
        ...
        height: Utilities.positiveInteger(height, 150)
      }
    );
  }
}
...
class Controller {
  static async run(inputDataParameters) {
    ...
    const modelData = ModelData.create();
...
class App {
  static async run() {
    ...
    const inputDataParameters = InputDataParameters.getDefault();
